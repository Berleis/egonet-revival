...
$DefaultGamePath = "C:\Program Files (x86)\Steam\steamapps\common\DiRT 4"
...
Write-Host "EgoNet Revival - DiRT 4 installer"
...
$runningGame = Get-Process -Name "dirt4" -ErrorAction SilentlyContinue
if ($runningGame) {
    Write-Host "Closing DiRT 4 before patching..."
...
foreach ($line in $existingHosts) {
    $trimmed = $line.Trim()
    if ($trimmed -eq "# EgoNet Revival DiRT 4") {
        continue
    }
...
$cleanHosts.Add("# EgoNet Revival DiRT 4") | Out-Null
...
Patch-Executable -ExecutablePath (Join-Path $GamePath "dirt4.exe") -RootBytes $rootBytes

Write-Host "Testing HTTPS health endpoint..."
& curl.exe -fsS --ssl-no-revoke "https://prod.egonet.codemasters.com/health" | Out-Host

Write-Host ""
Write-Host "Done. Open DiRT 4 and enter RaceNet."
